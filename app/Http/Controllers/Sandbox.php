...
class Sandbox extends Controller
{
    ...
    public function report($start)
    {
       ...
       { 
          $select = DB::select("SELECT  *
                                ...
                                ORDER BY id");
               return view('notallow.add.see_report',compact('select'))->with('start',0);
           
       }
       else
       {
           if($start<=2025||$start>2018)
           {
                
                $select = DB::select("SELECT *
                                FROM beta_report
                                WHERE YEAR(created_at) = '".$start."' 
                                ORDER BY id");
               return view('notallow.add.see_report',compact('select'))->with('start',$start);
           }
           else
           {
               dd("ປ້ອນປີຜິດ");
    ...
    public function report_edit($start)
    {
       if($start==0)
       { 
          $select = DB::select("SELECT  *
                                ...
                                WHERE YEAR(created_at) = '".date('Y')."' 
                                ORDER BY id");
               return view('notallow.add.see_report_edit',compact('select'))->with('start',0);
       ...
       else
       {
           if($start<=2025||$start>2018)
           {
                $select = DB::select("SELECT *
                                ...
                                WHERE YEAR(created_at) = '".$start."' 
                                ORDER BY id");
               return view('notallow.add.see_report_edit',compact('select'))->with('start',$start);
           }
           else
           {
               dd("ປ້ອນປີຜິດ");
    ...
    public function report_update(Request $request)
    {
        ...
        $beta_report = beta_report::where('id',$id)->first();
        $beta_report->total_unit = $val;
        $beta_report->save();
        
        return response()->json(['message' => 'success']);
        
    }
     public function cam_scan()
    {
        return view('sandbox.camera');
    }
}
