...
<body class="bg-slate-50 font-sans text-slate-950 lg:overflow-hidden">
    <div id="taskShell" class="min-h-screen lg:grid lg:h-screen lg:grid-cols-[280px_1fr] lg:overflow-hidden">
        <aside class="border-b border-blue-100 bg-white lg:flex lg:h-screen lg:flex-col lg:overflow-y-auto lg:border-b-0 lg:border-r">
            ...
            <form data-sidebar-hide method="POST" action="{{ route('user-task.logout') }}" class="px-4 pb-5 lg:mt-auto lg:px-5">
                @csrf
                ...
                <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-9">
                    ...
                    <div>
                        ...
                        <select id="status" class="filter-input h-10 w-full rounded-md border border-blue-100 bg-white px-3 text-sm outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100">
                            <option value="">ວຽກປັດຈຸບັນ</option>
    ...
    <script>
        const dataUrl = @json(route('user-task.dashboard.data'));
        const actionUrl = @json(route('user-task.dashboard.action'));
        const pointingUrl = @json(route('user-task.dashboard.pointing'));
        const subjectUrl = @json(route('user-task.dashboard.subjects'));
        const printUrlBase = @json(url('/see/enter/print'));
        ...
        const autoReloadStorageKey = 'user_task_dashboard_auto_reload';
        const autoReloadDateKey = 'user_task_dashboard_auto_reload_date';
        const sidebarStorageKey = 'user_task_dashboard_sidebar_collapsed';
        let rowsById = {};
        ...
        function renderStatusOptions(statuses) {
            ...
            statusSelect.innerHTML = `
                <option value="">ວຽກປັດຈຸບັນ</option>
                ${statuses.map(status => `<option value="${esc(status)}">${esc(statusLabel(status))}</option>`).join('')}
            `;
        ...
        function render(rows) {
            ...
            rowsBody.innerHTML = rows.map(row => `
                <tr class="align-top hover:bg-blue-50/50">
                    <td class="px-4 py-4 font-medium text-slate-950">${esc(row.enter_number || '-')}</td>
                    <td class="px-4 py-4">
                        <div class="space-y-2">
                ...
                <article class="p-4">
                    <div class="mb-3 flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-slate-950">${esc(row.enter_number || '-')}</p>
                        </div>
        ...
        function actionButtons(row) {
            ...
            return `
                <div class="flex flex-wrap items-center gap-2">
                    ${actions.map(action => `
                        <button type="button" onclick="${action.id === 'open_pointing' ? `openPointing(${Number(row.enter_id)})` : `runAction(${Number(row.enter_id)}, '${action.id}')`}" class="rounded-md border ${action.kind === 'back' ? 'border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100' : 'border-blue-100 bg-blue-50 text-blue-700 hover:bg-blue-100'} px-2 py-1 text-xs font-medium transition">
                            ${esc(action.label)}
                        </button>
                    `).join('')}
                    ${print}
        ...
        function printButton(row) {
            ...
            return `
                <a href="${printUrlBase}/${Number(row.enter_id)}" target="_blank" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-blue-100 bg-white text-slate-500 transition hover:bg-blue-50" title="ພິມ">
        ...
        function availableActions(status) {
            const actionsByRole = {
                ...
                '3': {
                    ...
                    READY: [{ id: 'back_signed', label: 'ດຶງກັບ', kind: 'back' }],
                },
                '4': {
        ...
        async function runAction(enterId, action) {
            ...
            try {
                const response = await fetch(actionUrl, {
                    ...
                    body: JSON.stringify({ enter_id: enterId, action }),
                });
        ...
        function openPointing(enterId) {
            ...
            saveAutoReload();
            stopAutoReload();
            document.getElementById('pointingModalTitle').textContent = `ລະບຸເສັ້ນທາງ - ${row.enter_number || enterId}`;
            document.getElementById('pointingModalSubtitle').textContent = row.com_name || '';
            document.getElementById('pointingProducts').textContent = [...new Set((row.details || []).map(detail => detail.p_import).filter(Boolean))].join(', ') || '-';
