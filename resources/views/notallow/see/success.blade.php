                    ...
                    <div class="p-1">
                        ...
                        <div class="dream  items-center"> 
                            ...
                            <div class="">
                                ...
                                <input type="text" class="form-control" id="text-search" placeholder="ຄົ້ນຫາບໍລິສັດ, ເລກທີ່, ບ້ານ, ເມືອງ, ແຂວງ" autocomplete="off" value="">
                                ...
                                <table class="table table-bordered  " style="width:100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>ຈັດການ</th> 
                                            <th>
                                                @if(auth()->check() && auth()->user()->email != '1@0.com')
                                                    ເອກະສານແນບ
                                                @endif        
                                            </th> 
                                        </tr>
                                ...
                                <div class=" d-block d-sm-block d-md-block d-lg-none space-y-3 mt-3">
                                    ...
                                    <div class="p-3 mt-3 bg-white border rounded-xl shadow-sm row_fade{{ $row_enter->enter_id }}">
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            <div>
                                                ...
                                                {{ date('d-m-Y',strtotime($row_enter->date_in)) }}
                                                </div>
                                            </div>
                                            ...
                                            <div class="text-right">
                                                <div class="d-flex justify-content-between">
                                                    <div class="flex items-center gap-2 flex-wrap">
                                                        {{-- Paste the SAME "ຈັດການ" logic here (dropdown + buttons) --}}
                                                        {{-- It will work because classes/data-id are same --}}
                                                        <div class="xdropdown">
                                                        <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2" viewBox="0 0 24 24" class="w-8 h-5 text-gray-400">
                                                        ...
                                                        {{-- Example: keep your primary quick buttons too (copy as-is) --}}
                                                        @if(auth()->user()->is_admin=='2')
                                                        @if($row_enter->status=='WAITING')
                                                            <button class="btn btn-light up" data-id="{{ $row_enter->enter_id}}">ຍອມຮັບ</button>
                                                        @elseif($row_enter->status=='READY')
                                                        ...
                                                        @if(auth()->user()->is_admin=='4' && $row_enter->status=='SIGNING')
                                                        <button class="btn btn-light sign" data-id="{{ $row_enter->enter_id}}">ລົງລາຍເຊັນ</button>
                                                        @endif
                                                        ...
                                                        @if(auth()->user()->is_admin=='3')
                                                            @if($row_enter->status=='POINTING')
                                                                ...
                                                                onclick="showOptionPointing({{ $row_enter->enter_id}},'{{ str_replace('ບ້ານ','',$row_enter->address) }}','{{ str_replace('ເມືອງ','',$row_enter->district) }}', '{{ str_replace('ແຂວງ','',$row_enter->province) }}','{{ $get_product_list_for_option }}')">
                                                                ສົ່ງໄປຖ້າເຊັນ
                                                                </button>
                                                            @elseif($row_enter->status=='SIGNINED')
                                                                <button class="btn btn-light ready" data-id="{{ $row_enter->enter_id}}">ສົ່ງຂາເຂົ້າຂາອອກ</button>
                                        ...
                                        @if($row_enter->status=='CANCEL')
                                            <div class="text-sm text-danger mt-1">ສາເຫດ : {{ $row_enter->cancel_log }}</div>
                                        @endif
                                        </div>
                                        ...
                                        <div class="text-sm">
                                            ...
                                            <div class="mt-1 text-sm text-end">
                                            @foreach ($beta_enter_road_detail as $row_enter_road_detail)
                                                @if($row_enter_road_detail->enter_id==$row_enter->enter_id)
                                        ...
                                        <div class="text-sm">
                                            ...
                                            @if($row_enter->lasttails)
                                            <div class="text-xs text-gray-500 mt-1 murphy-break-text">( {{ $row_enter->lasttails }} )</div>
                                            @endif
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if($row_enter->status=='SIGNINED' || $row_enter->status=='READY' || $row_enter->status=='SUCCESS')
                                                <!-- <a href="{{url('/see/enter/print/'.$row_enter->enter_id)}}" target="_BLANK" class="btn btn-light ">
                                    ...
                                    @endforeach
                                        ...
                                        <br><br>
                                        <div class="item-pagination d-flex justify-content-center mt-5 d-block d-sm-block d-md-block d-lg-none">
                                ...
                                <br>
                                <br>
                                    <div class="item-pagination d-flex justify-content-center mt-5 d-none d-sm-none d-md-none d-lg-block">
                                        {{ $beta_enter->links('pagination.custome-pagination') }}
                                    </div>
                                </div>
                            </div>
   ...
   <script>
    $('#text-search').on('keypress', function(e) {
        if (e.which === 13) { // 13 = Enter key
             ...
             window.location.href = "{{ url('/see/enter/success/search') }}/"+text_search;
        }
    });
