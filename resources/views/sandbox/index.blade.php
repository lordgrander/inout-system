...
<div class="components components-grid">
    ...
    <section>
    <header><center><h2  id="NavigationList">Sand bo<sub><asd  style="color:red">X</asd><br><small>M.U.R.P.H.Y</small></sub></h2></center></header>
    <h5>> Terminal Prompt : unroll Function completed ! !</h5>
    <h5 class="terminal-prompt">Terminal Prompt : </h5>
    <nav>
        <li>
            <a href="#"></a>
            <ul>
            <li><a href="#" onclick="return confirm('Sand?')" class="del_sand" id="clear_all">ລືບຂໍ້ມູນເຂັ້າອອກທັງໝົດ</a></li> 
            </ul>
            </li>
        </ul>
    </nav>
    ...
    <hr>
    <section>
        <button  class="btn btn-default " id="set_print_form">ໃຊ້ Form Print</button>
    </section>
    <hr>

    <section>
    <header><h2 id="Forms">ເລກທີ່</h2></header>
   
        <fieldset>
        <legend>Form legend</legend> 
        <div class="form-group">
            <label for="text"> </label>
            <input id="set_number" name="text" type="text" required="" minlength="5" placeholder="test" value="{{ $latestNumber }}" onchange="return confirm('Sand?')">
        </div> 
        </fieldset> 
    </section>
    <hr>
    <section>
    <header><h2 id="Forms">User</h2></header>
        <fieldset>
        <legend>Form legend</legend> 
        <div class="form-group">
            <label for="user_name"> .</label>
            <input id="user_name" name="user_name" type="text" placeholder="ຊື້" value="">
        </div>
        <div class="form-group">
            <label for="user_phone"> .</label>
            <input id="user_phone" name="user_phone" type="text" placeholder="ເບີໂທ" value="">
        </div>
        <div class="form-group">
            <label for="user_password"> .</label>
            <input id="user_password" name="user_password" type="text" placeholder="ລະຫັດຜ່ານ" value="">
        </div>
        <div class="form-group">
        ...
        <button class="btn btn-default add_sande"  >Submit</button>
        </fieldset>
    ...
    <section>
        <fieldset>
            ...
            @php($count=1)
                 @foreach ($beta_user as $row)
                <tr>
                    <td>{{ $count++ }}</td>
                    <td><code>{{ $row->is_admin }}</code></td>
                    <td>{{ $row->name }} | {{ $row->email }}</td>
                    <td><a href="#" class="del_add_sand" id="{{ $row->id }}" onclick="return confirm('Sand?')">x</a></td>
                </tr>
                @endforeach
            </table>
...
@if(auth()->user()->is_admin == 5)  
    ...
    $('.del_add_sand').on('click', function(){
        var data =  {  
                     "data" : $(this).attr('id'),
                 }
   ...
   $('#set_print_form').on('click', function(){ 
       var data =  {
                    "data" : $(this).val(),
                }
                ...
                $.ajax({
                    ...
                    dataType:"json",
                    success: function(response){ 
                       alert('ກຳລັງໃຊ້ຟອມ : '+response.message); 
                    }
                })
   });
