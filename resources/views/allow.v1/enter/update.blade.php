<x-app-layout>
                    ...
                    <div class="p-2 border-t border-gray-200 md:border-t-0 md:border-l">
                        <div class=" items-center">
                            <div class="ml-4 text-lg text-gray-600 leading-7 font-semibold">
                                ...
                                <div class="context"> 
                                    <input type="text" class="form-control" id="p_import" placeholder="ສິນຄ້ານຳເຂົ້າ">
                                </div>
                                <div class="context"> 
                                    <input type="text" class="form-control" id="detail" placeholder="ເລກທີນຳເຂົ້າສີນຄ້າ">
                                </div>
                                <div class="context">
                                    ...
                                    <div class="context">
                                        ...
                                        <button type="button" class="btn btn-success" id="add" value="ເພີ່ມ">
    ...
    <div class="py-2 laos">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white  table_display_data shadow-xl sm:rounded-lg">
                                ...
                                <tr>
                                    <td colspan="7">
                                        ...
                                        <div class="grid-container">
                                            <div class="grid-item">
                                                <input type="text" class="form-control address" id="address"
                                                    name="address" autocomplete="off" placeholder="ບ້ານ" value="{{ $beta_enter[0]->address }}">
                                            </div>
                                            <div class="grid-item">
                                                <input type="text" class="form-control district" id="district"
                                                    name="district" autocomplete="off" placeholder="ເມືອງ" value="{{ $beta_enter[0]->district }}">
                                            </div>
                                            <div class="grid-item">
                                                <input type="text" class="form-control province" id="province"
                                                    name="province" autocomplete="off" placeholder="ແຂວງ" value="{{ $beta_enter[0]->province }}">
                                            </div>
                                        </div>
                                        ...
                                        <input type="text" class="form-control lasttails" id="lasttails"
                                            ...
                                            placeholder="ລະບຸພິເສດສຳລັບປາຍທາງ ( ຖ້າຕ້ອງການ )" value="{{ $beta_enter[0]->lasttails }}">
                                        <br>
                                        ...
                                        <table class="table table-bordered">
                                            <tr>
                                                <td colspan="2">ເອກະສານແນບ</td>
                                            </tr>
                                        @php($count=1)
                                        @foreach ($beta_enter_file as $row_file) 
                                              
                                            <tr class="flie_row{{ $row_file->file_id }}">
                                                <td>
                                                    <a href="{{ asset($row_file->file_url)}}" target="_BLANK">
                                                    ເອກະສານ {{ $count++ }}
                                                        <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                                        stroke-width="2" viewBox="0 0 24 24" class=" h-4 ">
                                                ...
                                                <td width="5%">
                                                    <button class="btn btn-outline-danger delete_file" type="button" data-id="{{ $row_file->file_id }}">ລຶບ</button>
                                                </td>
                                            </tr>
                                        @endforeach
                                        </table>
    ...
    <script>
        $(document).ready(function () {
            ...
            @foreach ($beta_enter_detail as $row)
                ...
                weight = "{{ $row->weight}}";
                detail = "{{ $row->detail }}";
                display_t_model_data = arr_t_model.find(function (xdata) {
                ...
                var display_dum_data = '<tr class="row_' + index + '" id="display_' + index + '" style="display:">\
                                    <td><p id="display_plate_' + index + '">' + plate + '</p></td>\
                                    <td><p id="display_d_name_' + index + '">' + d_name + '</p></td>\
                                    <td><p id="display_t_model_' + index + '" data-value="' + t_model + '">"'+display_t_model_data.name+'"</p></td>\
                    ...
                    var input_dum_data = '<tr class="row_input_' + index + '" id="input_' + index + '" style="display:none">\
                    ...
                    $('#build').append(display_dum_data + input_dum_data);
                    SelectElement('input_t_model_'+index, {{ $row->t_model }} ) 
                }
            @endforeach
        ...
        $('#add').on('click', function (e) {
            ...
            if (rem == 5) {
                let dumx = "ເພີ່ມສູງສຸດ 5 ລາຍການ ( ທ່ານສາມາດອອກເອກະສານຕໍ່ໄດ້ຫຼັງຈາກສຳເລັດຊຸດປະຈຸບັນ )";
                showAlert(dumx);
                return false;
            } else {
                rem = rem + 1;
            ...
            let input_dum = '<tr class="row_input_' + index + '" id="input_' + index + '" style="display:none">\
                ...
                '" name="input_t_model_' + index + '" value="'+t_model+'">' + dum_input_option + '</select></td>\
                                <td><input type="text" class="form-control" id="input_p_import_' + index +
                '" name="input_p_import_' + index + '" value="' + p_import + '"></td>\
                                <td><input type="text" class="form-control" id="input_weight_' + index +
                '" name="input_weight_' + index + '" value="' + weight + '"></td>\
                                <td><input type="text" class="form-control" id="input_detail_' + index +
                '" name="input_detail_' + index + '" value="' + detail + '"></td>\
                                <td><asd class="edit_save" id="' + index +
            ...
            let province = $('#province').val();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        showAlert(dumxxx);
                        window.location.href = '{{url("/enter/list/view/".$id)}}';
            ...
            $('.delete_file').on('click',function(){
                ...
                // user clicked OK
                    ...
                    $.ajax({
                        type: "post",
                        url: "/enter/update/file/drop",
                        data: (i_file),
                        dataType: "json",
                        async: false,
                        cache: false, 
                        success: function (response) {
                            $('.flie_row'+file_id).fadeOut('fast');
                        },
                        complete: function () {
                            // me.data('requestRunning', false);
                        },
